...
export function middleware(req: NextRequest) {
  ...
  if (path.startsWith("/_next") || path.startsWith("/favicon") || path.startsWith("/robots.txt")) {
    return NextResponse.next();
  }

  // check header or cookie
  const header = req.headers.get("x-site-pass") || req.cookies.get("site-pass")?.value;
  if (header === PASS && PASS) return NextResponse.next();
